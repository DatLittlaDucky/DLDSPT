...
# --- Ensure Resources folder is in sys.path ---
...
MOD_LOGS_DIR = "mod_logs"
MOD_LAST_RUN_FILE = "mod_last_run.json"

def ensure_mod_logs_dir():
    if not os.path.isdir(MOD_LOGS_DIR):
        os.makedirs(MOD_LOGS_DIR, exist_ok=True)

def safe_network_action(action_desc):
    resp = input(f"Warning: Attempt to perform network action '{action_desc}'. Allow? (y/n): ").strip().lower()
    if resp != "y":
        raise PermissionError("Network action denied by user.")
...
def networking_menu():
    ...
    while True:
        ...
        print("3. Fetch public server list")
        print("4. Stop server (if running)")
        ...
        if choice == "q":
            ...
            if not HAVE_NETWORKING:
                print("Networking module missing.")
                input("Press Enter to continue...")
                continue
            safe_network_action("Host a server")
            port = input("Port to host on (default 5000): ").strip() or "5000"
            try:
                port = int(port)
            except ValueError:
                print("Invalid port, using 5000.")
                port = 5000
            pwd = input("Optional password (blank for none): ").strip() or None
            server_instance = networking.DLDSPTServer(port=port, password=pwd)
            ...
            if not HAVE_NETWORKING:
                ...
                input("Press Enter to continue...")
                continue
            safe_network_action("Connect to a server")
            host = input("Friend's IP or hostname: ").strip()
            ...
            try:
                ...
                print("Invalid port, using 5000.")
                port = 5000
            pwd = input("Password (blank if none): ").strip() or None
            client = networking.DLDSPTClient(host, port, password=pwd)
            try:
                client.connect()
                print(f"Connected to {host}:{port}. Type /quit to disconnect.")
                while True:
                    msg = input("Message: ")
                    if msg == "/quit":
                        try:
            ...
            if not HAVE_REQUESTS:
                ...
                input("Press Enter to continue...")
                continue
            safe_network_action("Fetch public server list")
            url = input("Enter server list URL (JSON): ").strip()
            import requests
            try:
                resp = requests.get(url, timeout=5)
                servers = resp.json()
                if not isinstance(servers, list):
                    print("Invalid server list format.")
                else:
                    for i, s in enumerate(servers, 1):
                        print(f"{i}. {s.get('host')}:{s.get('port')}")
            except Exception as e:
...
# --- Theme definitions for Rich UI ---
THEMES = {
    "light": {
        "title": "bold cyan",
        "subtitle": "italic green",
        "table_header": "bold cyan",
        "table_name": "bold green",
        "table_info": "yellow",
        "table_desc": "italic magenta",
        "panel": "white",
        "logo": "bold blue"
    },
    "dark": {
        "title": "bold white",
        "subtitle": "italic bright_cyan",
        "table_header": "bold bright_cyan",
        "table_name": "bold bright_green",
        "table_info": "bright_yellow",
        "table_desc": "italic bright_magenta",
        "panel": "grey23",
    ...
    "highcontrast": {
        ...
        "table_name": "bold bright_white",
        "table_info": "bright_yellow",
        "table_desc": "italic bright_white",
        "panel": "bright_black",
        "logo": "bold bright_yellow"
    },
    "lunar": {  # Renamed from purplepink to lunar
        "title": "bold magenta",
        "subtitle": "italic bright_magenta",
        "table_header": "bold magenta",
        "table_name": "bold bright_magenta",
        "table_info": "bright_magenta",
        "table_desc": "italic bright_magenta",
        "panel": "on magenta",
        "logo": "bold bright_magenta"
    }
}
...
def load_config():
    if os.path.isfile(CONFIG_FILE):
        try:
            ...
            pass
    # First run: run Package_Checker.py
    try:
        mods_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "Mods")
        pkg_checker = os.path.join(mods_path, "Package_Checker.py")
        if os.path.isfile(pkg_checker):
            print("Running Package_Checker for dependency check (first run)...")
            runpy.run_path(pkg_checker, run_name="__main__")
    except Exception as e:
        print(f"Could not run Package_Checker: {e}")
    # Default config
    return {
        "window_size": [800, 600],
        "pinned": [],
...
def list_mods(mods_path, sort_by="name"):
    # Animated spinner while scanning Mods folder
    stop_event = threading.Event()
    spinner_thread = threading.Thread(target=spinner, args=("Scanning Mods folder...", stop_event))
    ...
    try:
        ...
        for entry in os.listdir(mods_path):
            ...
            if os.path.isdir(full_path):
                # Directory is considered a mod if it contains __main__.py or manifest.json
                if (
                    os.path.isfile(os.path.join(full_path, "__main__.py"))
                    or os.path.isfile(os.path.join(full_path, "manifest.json"))
                ):
                ...
                for ext in SUPPORTED_EXTENSIONS:
                    if entry.lower().endswith(ext):
                        ...
                        break
        # Sorting
        return sorted(files, key=lambda f: os.path.basename(f).lower()) if sort_by == "name" else \
            sorted(files, key=lambda f: os.path.getmtime(f) if os.path.exists(f) else 0, reverse=True) if sort_by == "date" else \
            sorted(files, key=lambda f: os.path.getsize(f) if os.path.exists(f) else 0, reverse=True) if sort_by == "size" else \
            sorted(files, key=lambda f: (f not in set(config.get("favourites", [])), os.path.basename(f).lower())) if sort_by == "favourites" else files
    finally:
        stop_event.set()
        spinner_thread.join()
...
def open_guest_html(file_path, js_path=None):
    """
    Open HTML or JS mod in Google Chrome first, fallback to Edge.
    If js_path is provided, generate a temp HTML wrapper.
    """
    import subprocess
    import shutil
    import os
    ...
    if js_path:
        temp_html = os.path.join(tempfile.gettempdir(), "temp_js_wrapper.html")
...
<script src="{os.path.basename(js_path)}"></script>
    ...
    if chrome_path:
        try:
            subprocess.Popen([chrome_path, path_to_open])
            opened = True
        except Exception:
            opened = False

    # Try Edge if Chrome failed
    if not opened and edge_path:
        try:
            subprocess.Popen([edge_path, path_to_open])
            opened = True
        except Exception:
            opened = False
    ...
    if not opened:
        try:
            ...
            print("Could not open the HTML/JS file in Chrome, Edge, or default browser.")
    ...
    if latest:
        try:
            if version_tuple(DLDSPT_VERSION) < version_tuple(latest):
                # Removed warning sentence about running older version
...
def display_menu(py_files, filter_text=None, sort_by="name"):
    ...
    if USE_RICH:
        from rich.text import Text
        from rich.panel import Panel
        logo_text = Text(ASCII_LOGO, style=theme_cfg["logo"])
        console.print(logo_text)
        # Partnership banner in magenta if theme is lunar
        if theme == "lunar":
            console.print(Panel(Text(partnership_text, style="bold bright_magenta"), style="on magenta"))
        else:
            console.print(Panel(Text(partnership_text, style="bold magenta"), style=theme_cfg["panel"]))
        # ...existing code...
    else:
        print(ASCII_LOGO)
        ...
        print(partnership_text)
        # ...existing code...

    console.print(f"[{theme_cfg['table_info']}]Available Mods:[/{theme_cfg['table_info']}] {len(py_files)}")
    console.print(f"[{theme_cfg['table_info']}]Total Mods Size:[/{theme_cfg['table_info']}] {get_total_mods_size(py_files)}KB")
    console.print(f"[{theme_cfg['table_info']}]Sort By:[/{theme_cfg['table_info']}] {sort_by.capitalize()}")
    if last_ran_mod:
        console.print(f"[{theme_cfg['table_info']}]Last Ran Mod:[/{theme_cfg['table_info']}] {last_ran_mod}")
    console.print()

    filtered_files = py_files
    if filter_text:
        filtered_files = [f for f in py_files if filter_text.lower() in format_name(f).lower()]
        console.print(f"[{theme_cfg['table_desc']}]Filter: '{filter_text}' ({len(filtered_files)} mods shown)[/{theme_cfg['table_desc']}]")

    from rich.table import Table
    table = Table(show_header=True, header_style=theme_cfg["table_header"])
    table.add_column("#", style="dim", width=4)
    table.add_column("Name", style=theme_cfg["table_name"])
    table.add_column("Info", style=theme_cfg["table_info"])
    table.add_column("Description", style=theme_cfg["table_desc"])
    ...
    for i, f in enumerate(filtered_files, 1):
        ...
        last_run = get_last_run_time(f)
        table.add_row(str(i), format_name(f), info, desc or "-", hash_val, last_run)
    ...
    # console.print("[cyan]rs[/cyan] Restart DLDSPT")  # Removed restart option
    console.print("[cyan]fav[/cyan] favourite/unfavourite a mod")
    console.print("[cyan]pin[/cyan] pin/unpin a mod")
    console.print("[cyan]network[/cyan] Connect to a server or host one")
    console.print("[cyan]theme[/cyan] change the look of dldspt")
    # End of Rich UI block
    ...
    if not USE_RICH:
        ...
        for i, f in enumerate(filtered_files, 1):
            ...
            hash_val = get_file_hash(f)
            last_run = get_last_run_time(f)
            print(f"{i}. {format_name(f)} ({info}) [Hash: {hash_val}] [Last Run: {last_run}]")
            if desc:
                print(f"    ↳ {desc}")
        print("\nr. Reload mod list")
        ...
        # print("Type 'rs' to restart DLDSPT")  # Removed restart option
...
def show_update_log():
    print("\n--- Updates for V6 ---\n")
    print("1. Bug fix for subprocess opening webpage and then auto closing. (This was not a reported bug)")
    print("2. Resources folder is now always added to sys.path for mod/resource imports.")
    print("3. Package_Checker.py runs automatically on first run to check dependencies.")
    print("4. Package_Checker.py now asks to install any missing packages for you.")
    print("5Animated Loading/Progress – When running mods or scanning folders.")
    print("6. We have partnered with Lunar's Mods!")
    print("7. Added a cooler purple and pink design to the main menu (select theme 'purplepink').")
    print("\n--- End of Updates ---")
    input("Press Enter to continue...")
...
def set_theme():
    print("Available themes: light, dark, highcontrast, lunar")
...
def run_duckylang_script(mods_path, modname=None, script_path=None):
    # Resources path is already in sys.path from above
    if script_path:
        try:
            try:
                resources_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "Resources")
        ...
        with open(real_path, "rb") as f:
            while True:
                ...
                if not chunk:
                    break
                h.update(chunk)
        return h.hexdigest()[:12]  # Shorten for display
    except Exception:
        return "-"

def load_last_run_times():
    try:
        if os.path.isfile(MOD_LAST_RUN_FILE):
            with open(MOD_LAST_RUN_FILE, "r", encoding="utf-8") as f:
                return json.load(f)
    except Exception:
        pass
    return {}
...
def save_last_run_times(times):
    try:
        with open(MOD_LAST_RUN_FILE, "w", encoding="utf-8") as f:
            json.dump(times, f, indent=2)
    except Exception:
        pass

mod_last_run_times = load_last_run_times()

def update_last_run_time(mod_path):
    mod_last_run_times[mod_path] = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    save_last_run_times(mod_last_run_times)

def get_last_run_time(mod_path):
    return mod_last_run_times.get(mod_path, "-")

def log_mod_error(mod_path, error_info):
    ensure_mod_logs_dir()
    mod_name = os.path.basename(mod_path)
    log_file = os.path.join(MOD_LOGS_DIR, f"{mod_name}.log")
    try:
